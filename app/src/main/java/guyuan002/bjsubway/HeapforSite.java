...
public class HeapforSite {
    ...
    private void upHeap(int i) {
        int current = i;
        int parent = (i-1)/2;
        if ((i>0) && (bag.get(current).siteNumber > bag.get(parent).siteNumber)) {
    ...
    private void downHeap(int i) {
        ...
        if ((leftChild < bag.size()) && (bag.get(leftChild).siteNumber > bag.get(smaller).siteNumber)) smaller = leftChild;
        if ((rightChild < bag.size()) && (bag.get(rightChild).siteNumber > bag.get(smaller).siteNumber)) smaller = rightChild;
        if (smaller != i) {

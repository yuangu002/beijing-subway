 ...
 * */
public class HeapforTransfer {

    public ArrayList<Station> bag;


    public HeapforTransfer() {
    ...
    private void upHeap(int i) {
        int current = i;
        int parent = (i-1)/2;
        if ((i>0) && (bag.get(current).num < bag.get(parent).num)) {
    ...
    private void downHeap(int i) {
        ...
        if ((leftChild < bag.size()) && (bag.get(leftChild).num < bag.get(smaller).num)) smaller = leftChild;
        if ((rightChild < bag.size()) && (bag.get(rightChild).num < bag.get(smaller).num)) smaller = rightChild;
        if (smaller != i) {
